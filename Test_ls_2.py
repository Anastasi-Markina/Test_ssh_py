...
class TestPositive:
    ...
    def test_step1(self, make_folders, make_files):
        result1 = ssh_checkout(f"{data['host']}", f"{data['user']}", f"{data['password']}",
    ...
    def test_step2(self, make_files):
        result1 = ssh_checkout(f"{data['host']}", f"{data['user']}", f"{data['password']}",
                               ...
                               f"{data['valid']}")
        result2 = ssh_checkout(f"{data['host']}", f"{data['user']}", f"{data['password']}",
    ...
    def test_step5(self):
        assert ssh_checkout(f"{data['host']}", f"{data['user']}", f"{data['password']}",
    ...
    def test_step8(self, make_files, make_sub_folder):
        result2 = ssh_checkout(f"{data['host']}",
                 f"{data['user']}",
                 f"{data['password']}",
                               f"cd {data['folder_in']}; 7z a -t{data['type']} {data['folder_out']}/arx2",
                               f"{data['valid']}")
        result1 = ssh_checkout(f"{data['host']}",
                 f"{data['user']}",
                 f"{data['password']}",
                               f"cd {data['folder_out']}; 7z d arx2.{data['type']}",
                               f"{data['valid']}")
